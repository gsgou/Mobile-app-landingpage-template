...
Quikkauf collects some Personal Data from its Users.
...
-   ### Displaying content from external platforms
    -   ### Google Maps SDK and Google Places API(s)
-   ### Handling payments
...
Among the types of Personal Data that Quikkauf collects, by itself or
through third parties, there are: Usage Data; payment data; geographic
position; email address; Precise location permission (non-continuous).
...
Data, collected automatically when using Quikkauf.\
Unless specified otherwise, all Data requested by Quikkauf is mandatory
and failure to provide this Data may make it impossible for Quikkauf to
provide its services. In cases where Quikkauf specifically states that
...
Any use of Cookies -- or of other tracking tools -- by Quikkauf or by
the owners of third-party services used by Quikkauf serves the purpose
...
published or shared through Quikkauf and confirm that they have the
...
with the operation of Quikkauf (administration, sales, marketing, legal,
...
the following: Platform services and hosting, Handling payments,
Location-based interactions, Beta Testing, Device permissions for
Personal Data access, Analytics and Displaying content from external
platforms.
...
Depending on the User\'s specific device, Quikkauf may request certain
...
proper functioning of Quikkauf.
...
Used for accessing the User\'s precise device location. Quikkauf may
collect, use, and share User location Data in order to provide
location-based services.\
The geographic location of the User is determined in a manner that
isn\'t continuous. This means that it is impossible for Quikkauf to
...
-   ### Analytics
    ...
    App Center is a analytics and diagnostics service provided by
    Microsoft Corporation. Quikkauf utilizes the Data collected to
...
-   ### Beta Testing
    ...
    Quikkauf, or parts of it, for the purpose of testing a certain
    feature or the entire Application.\
    The service provider may automatically collect data related to
    crashes and statistics related to the User\'s use of Quikkauf in a
...
-   ### Device permissions for Personal Data access
    ...
    Quikkauf requests certain permissions from Users that allow it to
    access the User\'s device Data as described below.

    #### Device permissions for Personal Data access (Quikkauf)

    Quikkauf requests certain permissions from Users that allow it to
    access the User\'s device Data as summarized here and described
    within this document.

    Personal Data processed: Precise location permission
    (non-continuous).
...
-   ### Displaying content from external platforms
    ...
    platforms directly from the pages of Quikkauf and interact with
    them.\
    ...
    Google Maps SDK is a maps visualization service provided by Google
    Ireland Limited that allows Quikkauf to incorporate content of this
    kind on its pages.\
    [Google\'s privacy policy applies](https://policies.google.com/privacy).

    #### Google Places API(s)

    Google Places API(s) is a search for places within a specified area
    service provided by Google Ireland Limited that allows Quikkauf to
    incorporate content of this kind on its pages.\
    [Google\'s privacy policy applies](https://policies.google.com/privacy).

-   ### Handling payments

    Unless otherwise specified, Quikkauf processes any payments by
    ...
    information directly to such payment service providers. Quikkauf
    ...
    Quikkauf uses a payment service provided by Google Ireland Limited
    ...
    Quikkauf uses a payment service provided by Apple Inc. that allows
...
-   ### Location-based interactions
    ...
    #### Non-continuous geolocation (Quikkauf)

    Quikkauf may collect, use, and share User location Data in order to
    provide location-based services.\
    Most browsers and devices provide tools to opt out from this feature
    by default. If explicit authorization has been provided, the User's
    location data may be tracked by Quikkauf.\
    The geographic location of the User is determined in a manner that
    isn\'t continuous, either at the specific request of the User or
    when the User doesn\'t point out its current location in the
    appropriate field and allows the application to detect the position
    automatically.
...
-   ### Platform services and hosting
    ...
    components of Quikkauf, therefore allowing the provision of Quikkauf
    ...
    Quikkauf is distributed on the Google Play Store, a platform for the
    ...
    Quikkauf is distributed on Apple\'s App Store, a platform for the
    ...
    Quikkauf. Much of this information is processed on an opt-in basis.
...
improper use of Quikkauf or the related Services.\
...
Quikkauf may provide the User with additional and contextual information
concerning particular Services or the collection and processing of
Personal Data upon request.

### System logs and maintenance

For operation and maintenance purposes, Quikkauf and any third-party
services may collect files that record interaction with Quikkauf (System
...
Quikkauf does not support "Do Not Track" requests.\
...
Quikkauf and/or - as far as technically and legally feasible - sending a
...
by the business running Quikkauf and, if the case may be, its parent,
...
either directly or indirectly, from you when you use Quikkauf.

For example, you directly provide your personal information when you
submit requests via any forms on Quikkauf. You also provide personal
information indirectly when you navigate Quikkauf, as personal
...
If you'd like to know more, or exercise your right to opt out in regard
to all the sales carried out by Quikkauf, both online and offline, you
...
functioning of Quikkauf and features thereof ("business purposes"). In
...
and repair errors on Quikkauf, to detect security incidents and protect
...
This part of the document integrates with and supplements the
information contained in the rest of the privacy policy and is provided
by the entity running Quikkauf and, if the case may be, its parent,
subsidiaries and affiliates (for the purposes of this section referred
to collectively as "we", "us", "our").\
...
Information collected automatically through Quikkauf (or third-party
services employed in Quikkauf), which can include: the IP addresses or
domain names of the computers utilized by the Users who use Quikkauf,
...
The individual using Quikkauf who, unless otherwise specified, coincides
...
concerning the operation and use of Quikkauf. The Data Controller,
unless otherwise specified, is the Owner of Quikkauf.

#### Quikkauf (or this Application)
...
The service provided by Quikkauf as described in the relative terms (if
